...
class Connect:
    ...
    def __init__(self, login: str, code: str, host: str = 'i.protab.cz'):
        ...
        assert isinstance(login, str) and isinstance(code, str), 'login a code musí být stringy'
    ...
    def _get_int(self, command, *args) -> int:
        response = self._communicate(command, *args)
        if response == False:
            raise MazeException(f"Chyba při vykonání příkazu {' '.join(args)}: {self.error}")
        assert response != True, f'Neočekávaná odpověď'
    ...
    def get(self, x: int, y: int) -> int:
        ...
        assert isinstance(x, int), 'x musí být integer'
        assert isinstance(y, int), 'y musí být integer'
    ...
    def get_all(self):
        ...
        if data == False:
            raise MazeException(f"Chyba v get_all: {self.error}")
        assert data != True
        result = []
    ...
    def move(self, key: str) -> bool:
        ...
        assert isinstance(key, str) and len(key) == 1, 'key musí být jeden znak'
